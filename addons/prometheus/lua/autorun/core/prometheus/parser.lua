...
local Tokenizer = include("tokenizer.lua");
...
function Parser:expressionOr(scope)
	...
	if(consume(self, TokenKind.Keyword, "or") or consume(self, TokenKind.Keyword, "||")) then
...
function Parser:expressionAnd(scope)
	...
	if(consume(self, TokenKind.Keyword, "and") or consume(self, TokenKind.Keyword, "&&")) then
...
function Parser:expressionComparision(scope)
	...
	repeat
		...
		if(consume(self, TokenKind.Symbol, "~=") or consume(self, TokenKind.Symbol, "!=")) then
			local rhs = self:expressionStrCat(scope);
