...
function Tokenizer:new(settings) 
	local conventions = Tokenizer.Conventions
...
function Tokenizer:skipComment()
	if(is(self, "-", 0) and is(self, "-", 1)) then
		...
		return true;
	elseif (is(self, "/", 0)) then
		if is(self, "/", 1) then
			self.index = self.index + 2;
			while(self.index < self.length and self:parseAnnotation() ~= "\n") do end
			return true
		elseif is(self, "*", 1) then
			self.index = self.index + 2;
...
function Tokenizer:scanAll()
	...
	logger:info(string.format("Tokenization Done in: %.2f seconds", SysTime() - StartT));
	return tb
end
